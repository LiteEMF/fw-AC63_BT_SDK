...
#if defined LITEEMF_ENABLED && TCFG_USER_BLE_ENABLE
...
//ATT发送的包长,    note: 23 <= ATT_LOCAL_MTU_SIZE need >= MTU
// ATT_LOCAL_MTU_SIZE 应该是BLE支持的MTU,ATT以这个MTU为最大发送包,必须大于APP定义的MTU, 但是可以小于(浪费带宽)/大于(底层重新分包)底层PUD_PAYLOAD
#ifdef BT_ATT_MTU_SIZE            //考虑到不只是BLE数据通讯,还有其他的协议,这里只保证BT_MTU_SIZE 大于默认值才修改
#define ATT_LOCAL_MTU_SIZE        (BT_ATT_MTU_SIZE+3)   
#else
#define ATT_LOCAL_MTU_SIZE        (64)
#endif

//ATT缓存的buffer支持缓存数据包个数
#define ATT_PACKET_NUMS_MAX       (6)       //fix config_btctler_le_rx_nums 这是moredata是3, 这里缓存6个包相当于缓存两个链接的数据
